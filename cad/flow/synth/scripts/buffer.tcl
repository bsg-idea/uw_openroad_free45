...
set in_v_file    $::env(SYNTH_BUFFER_IN_V)
set in_sdc_file  $::env(SYNTH_BUFFER_IN_SDC)
set out_v_file   $::env(SYNTH_BUFFER_OUT_V)
set out_sdc_file $::env(SYNTH_BUFFER_OUT_SDC)
...
read_sdc $in_sdc_file

puts "Max Fanout Settings: $max_fanout"
...
# ------------------------------------------------------------------------------
set tiehi_cell {LOGIC1_X1 Z}
set tieHiCellName [lindex $tiehi_cell 0]
set tieHiCellOutPin [lindex $tiehi_cell 1]
...
foreach netObj $tieHiHighFanoutNetObjs {
  ...
  set numTieHiCells [expr round(ceil([llength $tieHiSinkPinObjs] * 1.0 / $max_fanout))]
  ...
  for {set i 0} {$i < $numTieHiCells} {incr i} {
    ...
    set loopLimit [expr min($max_fanout,[expr [llength $tieHiSinkPinObjs] - $tieHiConnectCnt])]
...
# Setup tie cell information
# ------------------------------------------------------------------------------
set tielo_cell {LOGIC0_X1 Z}
set tieLoCellName [lindex $tielo_cell 0]
set tieLoLibCell [get_lib_cells */$tieLoCellName]
set tieLoCellOutPin [lindex $tielo_cell 1]
...
foreach netObj $tieLoHighFanoutNetObjs {
  ...
  set numTieLoCells [expr round(ceil([llength $tieLoSinkPinObjs] * 1.0 / $max_fanout))]
  ...
  for {set i 0} {$i < $numTieLoCells} {incr i} {
    ...
    set loopLimit [expr min($max_fanout,[expr [llength $tieLoSinkPinObjs] - $tieLoConnectCnt])]
    for {set j 0} {$j < $loopLimit} {incr j} {
...
# ------------------------------------------------------------------------------
set min_buf {BUF_X1 A Z}
set bufferCell [lindex $min_buf 0]
set bufferLibCellObj [get_lib_cells */$bufferCell]
set bufferCellInPin [lindex $min_buf 1]
set bufferCellOutPin [lindex $min_buf 2]
...
set clkNetObj [get_nets $clock_port]
if {$clkNetObj == ""} {
  puts "Error: Cannot find toplevel clock net - $clock_port. Check design configuration"
  exit -1
}
...
foreach netObj $highFanoutNetObjs {
  ...
  set bufferHier ""
  while {$iterator > $max_fanout} {
    set iterator [expr $iterator * 1.0 / $max_fanout]
  ...
  proc nextBuffer {curBuffer} {
    ...
    for {set i [expr $levels-1]} {$i >= 0} {incr i -1} {
      if {$incrDone} {
        ...
        } elseif {[lindex $curBuffer $i] == [expr $max_fanout - 1]} {
  ...
  while {$currentSinkCnt < $netSinkPinCnt} {
    ...
    set sinkConnectCnt [expr min($max_fanout,[expr $netSinkPinCnt - $currentSinkCnt])]
...
write_verilog $out_v_file
write_sdc $out_sdc_file

# final report
report_checks > $run_dir/reports/buffer_checks.rpt
report_tns    > $run_dir/reports/buffer_tns.rpt
report_wns    > $run_dir/reports/buffer_wns.rpt
